...
const ThemVoucherPage = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const kieuGiamGiaText = form.kieuGiamGia === 'PERCENT' ? 'Giảm theo phần trăm' : 
                           form.kieuGiamGia === 'FIXED' ? 'Giảm cố định' : 'Miễn phí ship';
    
    const confirmed = window.confirm(
      `Bạn có chắc chắn muốn tạo phiếu giảm giá "${form.tenPhieuGiamGia}"?\n\n` +
      `Mã: ${form.maPhieuGiamGia}\n` +
      `Kiểu: ${kieuGiamGiaText}\n` +
      `Số lượng: ${form.soLuong}\n` +
      `Thời gian: ${form.ngayBatDau} - ${form.ngayKetThuc}`
    );
