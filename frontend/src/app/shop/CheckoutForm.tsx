...
export default function CheckoutForm({
    ...
    // Thêm các hàm validate trên cùng file (ngoài component CheckoutForm)
    ...
    const validatePhone = (phone: string) => {
        ...
        if (!/^0\d{9}$/.test(phone)) return "Số điện thoại phải có 10 số và bắt đầu bằng 0";
        return "";
    };
            ...
            <form onSubmit={e => {
                ...
                <Box sx={{mb: 2, display: 'flex', gap: 2}}>
                    <Box sx={{flex: 1}}>
                        ...
                        <input
                            ...
                            onChange={e => {
                                ...
                                setCustomerInfo((info: any) => ({...info, name: value}));
                    ...
                    <Box sx={{flex: 1}}>
                        ...
                        <input
                            ...
                            onChange={e => {
                                const value = e.target.value;
                                setCustomerInfo((info: any) => ({...info, phone: value}));
                                const error = validatePhone(value);
                                setFieldErrors((prev: any) => ({...prev, phone: error}));
                                if (customerFormError) setCustomerFormError('');
                            }}
                            style={{
                                width: '100%',
                ...
                <Box sx={{mb: 2}}>
                    ...
                    <input
                        ...
                        onChange={e => {
                            const value = e.target.value;
                            setCustomerInfo((info: any) => ({...info, email: value}));
                            const error = validateEmail(value);
                            setFieldErrors((prev: any) => ({...prev, email: error}));
                            if (customerFormError) setCustomerFormError('');
                        }}
                        style={{
                            width: '100%',
                ...
                <Box sx={{display: 'flex', gap: 2, mb: 2}}>
                    <Button variant="outlined" onClick={handleAutoFillCustomerInfo}
                            sx={{flex: 1, color: '#388e3c', borderColor: '#388e3c', fontWeight: 700}}>
                        Tự động điền thông tin khách hàng
                    </Button>
                    <Button variant="contained" onClick={() => setShowNewAddressForm(true)}
                            sx={{flex: 1, background: '#1976d2', color: '#fff', fontWeight: 700}}>
                        Tạo địa chỉ mới
                    </Button>
                </Box>
