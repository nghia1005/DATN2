                ...
                {!isStaff && showCheckout && (
                    <Box sx={{ maxWidth: 1100, mx: 'auto', mt: 4, mb: 6, bgcolor: '#fff', borderRadius: 3, boxShadow: '0 2px 12px #b59d3a22', p: 4, display: 'flex', gap: 4 }}>
                        ...
                        <OrderSummary
                          ...
                          showQRSelector={showQRSelector}
                          selectedQR={selectedQR}
                          setShowQRSelector={setShowQRSelector}
                          setSelectedQR={setSelectedQR}
                          onPay={async () => {
                            // Validation email
                            if (!customerInfo.email || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(customerInfo.email)) {
                              setCustomerFormError('Vui lòng nhập email hợp lệ!');
                              return;
                            }
                            ...
                            try {
                              ...
                              if (!response.ok) {
                                ...
                                throw new Error(`Không thể tạo hóa đơn: ${response.status} - ${errorText}`);
                              }
