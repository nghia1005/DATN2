...
export function useShopPageLogic() {
    ...
    useEffect(() => {
        if (typeof window !== 'undefined' && cart.length >= 0) {
            try {
                console.log('🛒 useShopPageLogic: Saving cart to localStorage:', cart);
                localStorage.setItem('shop_cart', JSON.stringify(cart));
                console.log('🛒 useShopPageLogic: Cart saved, dispatching event');
                // Dispatch custom event để thông báo cho Header cập nhật
                window.dispatchEvent(new CustomEvent('cartUpdated'));
                console.log('🛒 useShopPageLogic: Event dispatched');
            } catch (error) {
                    ...
                    fetch(`http://localhost:8080/khach-hang/hien-thi`)
                      ...
                      .then(data => {
                        ...
                        if (kh) {
                          ...
                          console.error('Không tìm thấy khách hàng với tên:', user.tenKhachHang || 'undefined', user.tenTaiKhoan || 'undefined');
                          console.log('User object structure:', user);
                          console.log('Available user fields:', Object.keys(user));
                          // Fallback: sử dụng thông tin từ localStorage
                          setCustomerInfo({
                            name: user.tenKhachHang || user.tenTaiKhoan || 'Khách hàng',
                            email: user.email || '',
                            phone: user.soDienThoai || user.sdt || '',
                            city: '',
                            district: '',
                    ...
                    fetch(`http://localhost:8080/khach-hang/hien-thi`)
                        ...
                        .then(data => {
                            ...
                            if (kh) {
                              ...
                              console.error('Auto-fill Không tìm thấy khách hàng với tên:', user.tenKhachHang || 'undefined', user.tenTaiKhoan || 'undefined');
                              console.log('Auto-fill User object structure:', user);
                              console.log('Auto-fill Available user fields:', Object.keys(user));
                              // Fallback: sử dụng thông tin từ localStorage
                              setCustomerInfo(prev => ({
                                  ...prev,
                                  name: user.tenKhachHang || user.tenTaiKhoan || 'Khách hàng',
