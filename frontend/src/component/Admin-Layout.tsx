...
export default function AdminLayout({ children, pageTitle, activeMenu: propActiveMenu, activeSubMenu: propActiveSubMenu }: AdminLayoutProps) {
    ...
    // Kiểm tra quyền truy cập và lọc menu dựa trên vai trò
    useEffect(() => {
        ...
        if (u.vaiTro !== "NHAN_VIEN" && u.vaiTro !== "QUAN_TRI_VIEN") {
        ...
        const filteredMenu = allMenuItems.filter(item => {
            if (!item.roles) return u.vaiTro === "QUAN_TRI_VIEN";
            return item.roles.includes(u.vaiTro);
        });
        ...
        const allowedPaths = ['/', '/pos', '/NhanVien'];
        
        // Nếu là route được phép, không cần kiểm tra thêm
        if (allowedPaths.some(path => currentPath.startsWith(path))) {
            return;
        }
        
        // Kiểm tra quyền truy cập cho các route khác
        const currentMenuItem = allMenuItems.find(item => 
            (item.route && currentPath.startsWith(item.route)) ||
            (item.children && item.children.some(
                subItem => subItem.route && currentPath.startsWith(subItem.route.split('?')[0])
            ))
        );
        
        // Nếu không tìm thấy menu item hoặc không có quyền truy cập
        if (currentMenuItem && currentMenuItem.roles && !currentMenuItem.roles.includes(u.vaiTro)) {
    ...
    const handleMenuClick = (menuId: string) => {
        if (menuId === "dashboard") {
            ...
            // Chuyển thẳng đến trang hóa đơn tại quầy
            router.push("/invoices/ofline");
            return;
