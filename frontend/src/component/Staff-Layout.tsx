...
import { useTheme, useMediaQuery, Typography, Avatar } from '@mui/material';
...
interface StaffLayoutProps {
    children: ReactNode;
    activeMenu?: string;
    onMenuChangeAction?: (menu: string) => void;
    pageTitle?: string;
    activeSubMenu?: string;
}

// Danh sách menu dành cho nhân viên
...
export default function StaffLayout({ 
    ...
    activeSubMenu: propActiveSubMenu 
}: StaffLayoutProps) {
                            ...
                            >
                                ...
                                {item.children && !collapsed && (
                                    <span style={{ 
                                        ...
                                        {(item.id === 'products' && shouldOpenProductMenu) || (item.id === 'invoices' && shouldOpenInvoicesMenu) ? '▼' : '▶'}
                                    </span>
                                )}
                            </button>
                            {item.children && ((item.id === 'products' && shouldOpenProductMenu) || (item.id === 'invoices' && shouldOpenInvoicesMenu)) && !collapsed && (
                                <div style={{
                                    marginLeft: 28,
                                    marginRight: 10,
            ...
            <div style={{ 
                ...
                <header style={{
                    ...
                    <h1 style={{ 
                        ...
                        {pageTitle || staffMenuItems.find(item => item.id === activeMenu)?.label || 'Quản lý'}
                    </h1>
                    {user && (
                        <div style={{ 
                            display: 'flex', 
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column' }}>
                                <span style={{ 
                                    ...
                                    {user.tenNhanVien || user.tenTaiKhoan || user.tenKhachHang || 'Nhân viên'}
                                </span>
                                <span style={{ 
                                    fontWeight: 500, 
                                    color: '#8a7a2a', 
